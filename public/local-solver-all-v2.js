...
(function(){
  ...
  function normalizePayload(p){
    ...
    out.loads=(p.loads||[]).map(l=>{
      if(l.type==='point'){
        const raw=n(l.magnitude??l.value??0),angle=n(l.angle??0),vertical=Number.isFinite(angle)?raw*Math.cos(angle*Math.PI/180):raw,x=n(l.position??l.from);
  ...
  function buildStatics(payload,result){
    ...
    for(let i=0;i<cuts.length-1;i++){
      ...
      push(shear,a,V(a,false));push(moment,a,M(a,false));
      for(let k=1;k<steps;k++){const x=a+(b-a)*k/steps;push(shear,x,V(x,false));push(moment,x,M(x,false));}
      const vl=V(b,true),vr=V(b,false),ml=M(b,true),mr=M(b,false);
      push(shear,b,vl);if(!near(vl,vr))push(shear,b,vr);
      if(hinges.some(h=>near(h,b))){push(moment,b,0);push(moment,b,0);}else{push(moment,b,ml);if(!near(ml,mr))push(moment,b,mr);}
    }
    ...
    const stable=arr=>arr.map((p,i)=>({p,i})).sort((a,b)=>{const dx=a.p[0]-b.p[0];return Math.abs(dx)>EPS?dx:a.i-b.i;}).map(x=>x.p);
    const out={...result,diagrams:{...(result.diagrams||{}),shear:stable(shear),moment:stable(moment)}};
    out.meta={...(result.meta||{}),diagramEngine:'BeamAnalyzer-SinglePipeline-1.2'};
    return out;
  }
  ...
  window.fetch=async function(input,init){
    ...
    try{
      const payload=normalizePayload(JSON.parse(init.body)),solver=window.__beamAnalyzerSolveInternalHinge;
      ...
      return new Response(JSON.stringify(corrected),{status:200,headers:{'Content-Type':'application/json','Cache-Control':'no-store','X-Engine-Version':'BeamAnalyzer-SinglePipeline-1.2'}});
    }catch(error){
