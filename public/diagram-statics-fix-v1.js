...
(function(){
  ...
  function pointForce(l){
    const raw=n(l.value??l.magnitude??0);
    const ang=Number.isFinite(n(l.angle))?n(l.angle):0;
    return raw*Math.cos(ang*Math.PI/180);
  }

  function qArea(l,u,v){
    const a=n(l.from), b=n(l.to);
    const lo=Math.max(a,u), hi=Math.min(b,v);
    if(!(hi>lo+EPS))return 0;
    const q0=n(l.value??l.start??0), q1=n(l.value2??l.end??l.value??l.start??0);
    const m=(q1-q0)/(b-a);
  ...
  function qMomentToX(l,u,x){
    const a=n(l.from), b=n(l.to);
    const lo=Math.max(a,u), hi=Math.min(b,x);
    if(!(hi>lo+EPS))return 0;
    const q0=n(l.value??l.start??0), q1=n(l.value2??l.end??l.value??l.start??0);
    const m=(q1-q0)/(b-a);
  ...
  function buildDiagrams(payload,result){
    ...
    const cuts=uniqueSorted([
      0,total,...supports.map(s=>s?.position),...reactions.map(r=>r.x),
      ...points.map(p=>p.x),...allJumps.map(m=>m.x),...udls.flatMap(l=>[l.a,l.b])
    ].map(x=>Math.max(0,Math.min(total,n(x)))));

    function shearRight(x){
      let v=0;
      for(const r of reactions)if(r.x<=x+EPS)v+=r.f;
      for(const p of points)if(p.x<=x+EPS)v+=p.f;
      for(const l of udls)v+=qArea(l,l.from,x);
      return v;
    }

    function shearLeft(x){
      let v=0;
      for(const r of reactions)if(r.x<x-EPS)v+=r.f;
      for(const p of points)if(p.x<x-EPS)v+=p.f;
      for(const l of udls)v+=qArea(l,l.from,x-EPS);
      return v;
    }
    ...
    let Mstart=0;
    // A fixed support at the left boundary contributes its external reaction
    // moment with the opposite sign to the internal BMD convention.
    ...
    for(let c=0;c<cuts.length-1;c++){
      ...
      for(let k=0;k<steps;k++){
        ...
        push(shear,x,shearRight(x));
        push(moment,x,M);
      }

      const Mleft=Mstart + Va*dx
        + points.filter(p=>p.x>a+EPS&&p.x<=b+EPS).reduce((s,p)=>s+p.f*(b-p.x),0)
        + udls.reduce((s,l)=>s+qMomentToX(l,a,b),0);
      ...
      // Internal hinges are moment releases: the BMD must hit zero there.
      if(isHinge(b)){
        push(moment,b,Mleft);
        Mstart=0;
        push(moment,b,0);
    ...
    return {
      ...result,
      diagrams:{...result.diagrams,shear,moment},
      extremes:{...(result.extremes||{}),shear:extrema(shear),moment:extrema(moment)},
      meta:{...(result.meta||{}),diagramEngine:'BeamAnalyzer-Exact-Statics-2.0'}
    };
  }
