/* Beam Analyzer — authoritative internal-hinge renderer v5.
 *
 * IMPORTANT: app.js declares `model` with top-level `let`, so it is NOT
 * exposed as window.model. The previous v4 renderer therefore never saw the
 * internal hinge model and silently did nothing. This version accesses the
 * actual global lexical binding directly.
 *
 * The legacy renderer treats unknown support types as PINs. This renderer
 * suppresses the complete native support group for internal hinges and draws
 * an independent release symbol. Solver/model data is untouched.
 */
(function(){
  'use strict';
  ...
  style.textContent=`
    .beamCanvas g.internal-hinge-native-suppressed > *{display:none !important}
    .beamCanvas g.finalInternalHingeV5{display:inline !important}
  `;
  ...
  function hingeX(g,p){
    return g.x1+(Math.max(0,Math.min(g.L,num(p)))/g.L)*(g.x2-g.x1);
  }
  ...
  function suppressNative(svg,s,x){
    svg.querySelectorAll('g.supportDrag,g.supportGroup,g[data-support-id],g[data-id]').forEach(g=>{
      ...
      if(String(id)===String(s.id)||
         (Number.isFinite(num(bx))&&near(bx,x))||
         (Number.isFinite(num(gx))&&near(gx,x))){
        g.classList.add('internal-hinge-native-suppressed');
      }
    });

    // Also suppress legacy loose graphics produced by older UI patches.
    svg.querySelectorAll('.supportBadge').forEach(e=>{
      if(near(e.getAttribute('cx'),x))e.style.display='none';
    });
    svg.querySelectorAll('text.supportText').forEach(e=>{
      if(near(e.getAttribute('x'),x))e.style.display='none';
    });
  }

  function draw(svg,x,y,i,p){
    const g=document.createElementNS(NS,'g');
    g.setAttribute('class','finalInternalHingeV5');
    g.setAttribute('pointer-events','none');
    g.setAttribute('aria-label',`Internal Hinge H${i}`);

    // Dedicated internal-release symbol. It is intentionally not a triangle,
    // so it cannot be confused with the Pin/roller support graphics.
    ...
    title.textContent='Internal Hinge';
    g.appendChild(title);
    ...
    const unit=typeof unitText==='function'?unitText('length'):'';
    const val=typeof fmt==='function'?fmt(p):p;
    pos.textContent=`@ ${val} ${unit}`.trim();
    g.appendChild(pos);
  ...
  function repair(){
    ...
    svg.querySelectorAll('.finalInternalHingeV5').forEach(e=>e.remove());
    const hinges=m.supports
      .filter(s=>s&&s.type==='internal-hinge'&&Number.isFinite(num(s.position)))
    ...
    // Replace legacy support dimension marker S# with H#.
    hinges.forEach((s,i)=>{
      const x=hingeX(geo,s.position);
      svg.querySelectorAll('text.dimText').forEach(t=>{
        if(near(t.getAttribute('x'),x)){
          ...
          if(/^S\d+\s*:/.test(text)){
            t.textContent=text.replace(/^S\d+\s*:/,`H${i+1} :`);
          }
        }
      });
    });
