...
(function(){
  ...
  function renderDistributedLoad(l,x,beamY){
    ...
    const topPath=sameSign
      ? `<line x1="${a}" y1="${top1}" x2="${b}" y2="${top2}" class="udlEnvelope"/>`
      : `<polyline points="${a},${top1} ${(a+b)/2},${beamY-18} ${b},${top2}" class="udlEnvelope"/>`;
    const mid=(a+b)/2;
  ...
  window.renderBeam=function(){
    ...
    if(window.showDims !== false){
      ...
      let start=0;
      for(const s of window.model?.spans||[]){
    ...
    (window.model?.supports||[]).forEach((s,i)=>{
      const xx=x(s.position), type=s.type==='fixed'?'Fixed':s.type==='roller'?'Roller':'Pin';
      svg+=`<g class="drag supportDrag" data-drag="support" data-id="${s.id}">${beamSupport(s,xx,beamY)}
    ...
    (window.model?.loads||[]).forEach(l=>{
      if(l.type==='point'){
        ...
        svg+=renderDistributedLoad(l,x,beamY);
      }
    });
