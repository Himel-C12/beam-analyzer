...
(function(){
  ...
  function addAnnotations(card){
    const svg=card.querySelector('svg');if(!svg||typeof model==='undefined')return;
    svg.querySelector('.cleanDiagramAnnotations')?.remove();svg.querySelector('.importantValues')?.remove();svg.querySelector('.chartPointAnnotations')?.remove();
    const series=seriesFor(svg);if(!series.length)return;
    const kind=svg.dataset.kind||'',W=1100,H=330,pad=Number(svg.dataset.pad)||56,L=Math.max(Number(svg.dataset.len)||1,1),min=Number(svg.dataset.min),max=Number(svg.dataset.max);
    const sx=x=>pad+(x/L)*(W-2*pad),sy=y=>H-pad-(y-min)/(max-min||1)*(H-2*pad),items=[];
    const supports=Array.isArray(model.supports)?model.supports:[],loads=Array.isArray(model.loads)?model.loads:[];

    supports.forEach(s=>{const p=nearest(series,num(s.position));if(p)items.push({x:sx(p.x),y:sy(p.y),text:valueText(kind,p.y),anchor:'middle',cls:'supportValue'})});
    loads.forEach(l=>{
      const x0=num(l.from);if(!finite(x0))return;
      if(l.type==='point'&&kind==='shear'){const j=beforeAfter(series,x0);if(j.before&&j.after){items.push({x:sx(x0)-12,y:sy(j.before.y),text:valueText(kind,j.before.y),anchor:'end',cls:'jumpValue'});items.push({x:sx(x0)+12,y:sy(j.after.y)+18,text:valueText(kind,j.after.y),anchor:'start',cls:'jumpValue'})}return}
      if(l.type==='moment'&&kind==='moment'){const j=beforeAfter(series,x0);if(j.before&&j.after){items.push({x:sx(x0)-12,y:sy(j.before.y),text:valueText(kind,j.before.y),anchor:'end',cls:'jumpValue'});items.push({x:sx(x0)+12,y:sy(j.after.y)+18,text:valueText(kind,j.after.y),anchor:'start',cls:'jumpValue'})}return}
      if(l.type==='udl'&&(kind==='shear'||kind==='moment'))[num(l.from),num(l.to)].forEach((pos,i)=>{const p=nearest(series,pos);if(p)items.push({x:sx(p.x),y:sy(p.y),text:valueText(kind,p.y),anchor:i?'end':'start',cls:'boundaryValue'})});
    });

    let extreme=null;for(const p of series)if(!extreme||Math.abs(p.y)>Math.abs(extreme.y))extreme=p;
    if(extreme){const ex=sx(extreme.x),ey=sy(extreme.y),dup=items.some(i=>i.cls==='jumpValue'&&near((i.x-pad)/(W-2*pad)*L,extreme.x,1e-5));if(!dup)items.push({x:ex,y:ey,text:`Max: ${valueText(kind,extreme.y)}`,anchor:'middle',cls:'maxValue'})}

    const g=document.createElementNS('http://www.w3.org/2000/svg','g');g.setAttribute('class','cleanDiagramAnnotations');items.forEach(item=>addText(g,item));svg.appendChild(g);place(items,W,H,pad);
  }
  function annotate(){patchReactionTable();$$('#charts .chart').forEach(addAnnotations)}

  const style=document.createElement('style');style.textContent=`
    .cleanDiagramAnnotations{pointer-events:none}.cleanDiagramValue{fill:var(--text,#e7edf5);font-size:11px;font-weight:650;paint-order:stroke;stroke:var(--card,#101214);stroke-width:4px;stroke-linejoin:round}.cleanDiagramValue.jumpValue{font-size:11px;font-weight:750}.cleanDiagramValue.maxValue{font-size:10.5px;font-weight:750}.cleanDiagramValue.supportValue{font-size:10px;font-weight:550;opacity:.9}`;document.head.appendChild(style);
